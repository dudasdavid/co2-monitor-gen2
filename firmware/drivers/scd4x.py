...
class SCD4X:
    ...
    START_LOW_POWER_PERIODIC_MEASUREMENT = const(0x21AC)
    READ_MEASUREMENT = const(0xEC05)
    SERIAL_NUMBER = const(0x3682)
    SET_PRESSURE = const(0x241D)
    GET_ALTITUDE = const(0x2322)
    SET_ALTITUDE = const(0x2427)
    ...
    def serial_number(self):
        ...
        self._send_command(self.SERIAL_NUMBER, cmd_delay=0.001)
        self._read_reply(self._buffer, 9)
        return (
            self._buffer[0],
            self._buffer[1],
    ...
    def set_ambient_pressure(self, ambient_pressure): 
        ...
        self._set_command_value(self.SET_PRESSURE, int(ambient_pressure))
    ...
    def get_altitude(self):
        ...
        .. note::
            ...
            persist_settings().
        """
        self._send_command(self.GET_ALTITUDE, cmd_delay=0.001)
        self._read_reply(self._buffer, 3)
        return (self._buffer[0] << 8) | self._buffer[1]

    def set_altitude(self, height):
        if height > 65535:
            raise AttributeError("Height must be less than or equal to 65535 metres")
        self._set_command_value(self.SET_ALTITUDE, int(height))
    ...
    def start_low_periodic_measurement(self):
        ...
        for more details.
        """
        self._send_command(self.START_LOW_POWER_PERIODIC_MEASUREMENT, cmd_delay=0.01)
